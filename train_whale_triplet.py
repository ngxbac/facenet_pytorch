...
# Model options
parser.add_argument('--dataroot', type=str, default='../../input/dlib_gen/',
                    help='path to dataset')
parser.add_argument('--lfw-dir', type=str, default='../../input/dlib_gen/',
                    help='path to dataset')
...
LOG_DIR = config.log_dir + '/run-optim_{}-n{}-lr{}-wd{}-m{}-embeddings{}-msceleb-alpha10'\
    .format(config.optimizer, config.n_triplets, config.lr, config.wd,
            config.margin, config.embedding_size)
...
train_dir = TripletWhaleDataset(dir=config.dataroot,n_triplets=config.n_triplets,transform=transform)
...
def main():
    ...
    model = TripletWhaleModel(embedding_size=config.embedding_size,
